...
| 7 | **Reminders** | Voice-dictated reminders in Urdu, NLU-extracted task/date/contact, morning digest with sent-state tracking | Steel-trading client build (`agents/` — private), plus `apps/restaurant-bot/reservations/reminders.py` for booking reminders | Built (client); Meta template approval pending for out-of-window sends |
