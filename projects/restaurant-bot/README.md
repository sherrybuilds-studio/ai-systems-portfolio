...
A conversational AI assistant for restaurants, delivered over Telegram. It handles food orders, table reservations, and menu/FAQ questions in multiple languages, answering from the restaurant's own menu data via retrieval-augmented generation (RAG) — so responses stay grounded in what the restaurant actually offers. Built as a productized service: one codebase, configured per restaurant, running 24/7 under process supervision.
